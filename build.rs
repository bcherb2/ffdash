...
fn validate_registry(registry: &toml::Value) {
    ...
    assert!(
        meta.get("ffmpeg_version").is_some(),
        "Missing ffmpeg_version in [meta]"
    );
    assert!(
        meta.get("last_verified").is_some(),
        "Missing last_verified in [meta]"
    );
    ...
    assert!(
        !encoders.is_empty(),
        "Must have at least one encoder defined"
    );
    ...
    for encoder in encoders {
        ...
        assert!(
            encoder_table.get("codec").is_some(),
            "Encoder '{}' missing 'codec' field",
            id
        );
        assert!(
            encoder_table.get("type").is_some(),
            "Encoder '{}' missing 'type' field",
            id
        );
        assert!(
            encoder_table.get("ffmpeg_name").is_some(),
            "Encoder '{}' missing 'ffmpeg_name' field",
            id
        );
        ...
        if let Some(encoder_type) = encoder_table.get("type").and_then(|v| v.as_str()) {
            if encoder_type == "hardware" {
                assert!(
                    encoder_table.get("hw_api").is_some(),
                    "Hardware encoder '{}' must have 'hw_api' field",
                    id
                );
            }
        }
        ...
        if let Some(params) = encoder_table.get("params").and_then(|p| p.as_array()) {
            ...
            for param in params {
                ...
                assert!(
                    ...
                    "Duplicate parameter '{}' in encoder '{}'",
                    param_id,
                    id
                );
                ...
                assert!(
                    param_table.get("flag").is_some(),
                    "Param '{}' in encoder '{}' missing 'flag' field",
                    param_id,
                    id
                );
                assert!(
                    param_table.get("type").is_some(),
                    "Param '{}' in encoder '{}' missing 'type' field",
                    param_id,
                    id
                );
                assert!(
                    param_table.get("description").is_some(),
                    "Param '{}' in encoder '{}' missing 'description' field",
                    param_id,
                    id
                );
            ...
            assert!(
                !param_ids.is_empty(),
                "Encoder '{}' must have at least one parameter defined",
                id
            );
        } else {
    ...
    println!(
        "cargo:warning=TOML validation passed: {} encoders, {} total parameter definitions",
        encoder_ids.len(),
        total_params
    );
}
...
fn generate_params_code() {
    ...
    let toml_content = fs::read_to_string(toml_path).expect("Failed to read encoder-params.toml");

    let registry: toml::Value =
        toml::from_str(&toml_content).expect("Failed to parse encoder-params.toml");
        ...
        for encoder in encoders {
            ...
            if let Some(params) = encoder_table.get("params").and_then(|p| p.as_array()) {
                for param in params {
                    ...
                    let scope = param_table
                        .get("scope")
                        .and_then(|v| v.as_str())
                        .unwrap_or("private");
                    let flag = param_table.get("flag").and_then(|v| v.as_str()).unwrap();
                    let param_type = param_table.get("type").and_then(|v| v.as_str()).unwrap();
                    let description = param_table
                        .get("description")
                        .and_then(|v| v.as_str())
                        .unwrap_or("");
                    let warning = param_table
                        .get("warning")
                        .and_then(|v| v.as_str())
                        .map(|s| s.to_string());
                    let field_path = param_table
                        .get("field_path")
                        .and_then(|v| v.as_str())
                        .unwrap_or(&param_id.replace("-", "_"))
                        .to_string();

                    // Parse range
                    let range = if let Some(range_table) =
                        param_table.get("range").and_then(|r| r.as_table())
                    {
                        if let (Some(min), Some(max)) = (
                            ...
                            range_table.get("values").and_then(|v| v.as_array())
                        {
                    ...
                    let default = if let Some(default_val) = param_table.get("default") {
                        match param_type {
                            ...
                            "enum" | "string" => {
                                DefaultValue::String(default_val.as_str().unwrap_or("").to_string())
                            }
    ...
    println!(
        "Generated {} encoders and {} parameters",
        encoder_ids.len(),
        params_map.len()
    );
}
